...
<div class="container">
    ...
    if($username!='' && $password!=''){
        ...
        echo "<center>Go to <a href='student_login.php'>Login</a> page</center>";
    }else{
...
{          
 ...
 $deldata = $db->delete($stmt);
        $stmt = $db->link->prepare("SELECT id,username FROM examinees_info where id=?");
  $stmt->bind_param("i",$userid);
  $singleSQL = $db->select($stmt);
              if($singleSQL){
             $value= array_shift($singleSQL);
             $username = $value['username'];
             echo '<form action="student_reset.php" method="post" id="reset" >
