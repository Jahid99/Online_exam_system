...
// $stmt = $db->link->prepare("INSERT INTO time_maintain (the_time) VALUES (?)");
...
$stmt = $db->link->prepare("SELECT * FROM time_maintain WHERE id=?");
  ...
  $starting_date = $dt->format('Y/m/d, H:i:s');
  $seconds = (strtotime($the_time)+$_SESSION['per_q_limit']) - strtotime($current_date);
